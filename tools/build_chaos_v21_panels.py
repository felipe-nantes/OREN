#!/usr/bin/env python3
"""Build label-blind CHAOS v21 panels or their technical gallery."""
...
from dtwin.benchmark.chaos_v21_panels import (
    build_chaos_uniform9_gallery,
    build_chaos_uniform9_panels,
)
...
def main() -> int:
    ...
    panels.add_argument("--expected-case-count", type=int, default=20)
    ...
    try:
        if args.command == "panels":
            result = build_chaos_uniform9_panels(
                prepared_root=args.prepared, output_root=args.out,
                config_path=args.config, profile_path=args.profile,
                expected_case_count=args.expected_case_count,
                expected_prepared_signature=args.expected_prepared_signature,
            )
        else:
            result = build_chaos_uniform9_gallery(panel_root=args.panels, output_dir=args.out)
