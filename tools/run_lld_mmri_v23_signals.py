#!/usr/bin/env python3
"""Run reviewed LLD-MMRI v23 signals in separate GPU-safe stages."""
...
from dtwin.benchmark.lld_mmri_v23_signals import (
...
class HTTPChoiceScorer:
    ...
    def score_panel(self, panel_path: Path, prompt: str):
        ...
        return {"choice_probabilities": self.client.last_response_audit.get("choice_probabilities")}
...
def main() -> int:
    ...
    manifest = commands.add_parser("localizer-manifest"); _common(manifest); manifest.add_argument("--out", type=Path, required=True)
    localizer = commands.add_parser("localizer"); _common(localizer); localizer.add_argument("--manifest", type=Path, required=True); localizer.add_argument("--out", type=Path, required=True)
    medsiglip = commands.add_parser("medsiglip"); _common(medsiglip); medsiglip.add_argument("--out", type=Path, required=True); medsiglip.add_argument("--device", choices=("cpu", "cuda"), default="cuda")
    medgemma = commands.add_parser("medgemma"); _common(medgemma); medgemma.add_argument("--out", type=Path, required=True)
    assemble = commands.add_parser("assemble"); _common(assemble); assemble.add_argument("--medgemma", type=Path, required=True); assemble.add_argument("--medsiglip", type=Path, required=True); assemble.add_argument("--localizer", type=Path, required=True); assemble.add_argument("--out", type=Path, required=True)
    ...
    try:
        context = verify_lld_mmri_v23_signal_context(**common)
        if args.command == "localizer-manifest":
            result = build_lld_mmri_v23_localizer_input_manifest(
            ...
            with block_optional_module_for_spawn("pyarrow") as guarded:
                result = run_localizer_scores(
                    ...
                    input_root=Path(args.prepared) / "inputs",
                    output_root=args.out,
                    case_ids=context["case_ids"],
                    localizer=TotalSegmentatorMRLesionLocalizer(),
                    expected_source_case_count=context["case_count"],
            ...
            result = run_v21_medsiglip_scores(
                context=context,
                panel_root=args.panels,
                output_root=args.out,
            ...
            result = run_v21_medgemma_scores(
                context=context,
                panel_root=args.panels,
                output_root=args.out,
            ...
            result = assemble_lld_mmri_v23_raw_signals(
                context=context,
