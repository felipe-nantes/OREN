#!/usr/bin/env python3
"""Run reviewed CHAOS v21 signals in separate GPU-safe stages."""
...
from dtwin.benchmark.chaos_v21_signals import (
    LOCALIZER_IMAGE_ROLE,
    LOCALIZER_MASK_ROLE,
...
def main() -> int:
    ...
    try:
        if args.command == "localizer-manifest":
            result = build_chaos_v21_localizer_input_manifest(**common, output_path=args.out)
        else:
            context = verify_chaos_v21_signal_context(**common)
            if args.command == "localizer":
                with block_optional_module_for_spawn("pyarrow") as guarded:
                    result = run_localizer_scores(
                        manifest_path=args.manifest, input_root=args.prepared,
                        output_root=args.out, case_ids=context["case_ids"],
                        localizer=TotalSegmentatorMRLesionLocalizer(),
                        expected_source_case_count=args.expected_case_count,
                ...
                result = run_v21_medsiglip_scores(
                    context=context, panel_root=args.panels,
                ...
                result = assemble_v21_raw_signals(
                    context=context, medgemma_root=args.medgemma,
                    medsiglip_root=args.medsiglip, localizer_root=args.localizer,
                    output_dir=args.out,
                )
