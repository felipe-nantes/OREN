"""Build the four-case, signed v16 timing-review bundle without reading labels."""
from __future__ import annotations

import shutil
import uuid
from pathlib import Path
from typing import Any

from dtwin.benchmark.openswisshcc_alignment import _publish_directory, _sha256
from dtwin.benchmark.openswisshcc_candidate_volume import (
    COHORT_SCHEMA,
    CONTRACT,
    MAX_CANDIDATES,
    MIN_BASE_CANDIDATES,
    OUTPUT_SIDE,
    ROI_MM,
    TARGET_CANDIDATE_COVERAGE,
    _canonical,
    _gallery_page,
    _input_index,
    _load,
    _original_dynamic_inputs,
    _registered_or_none,
    _valid_localizer_run_schema,
    build_candidate_volume_case,
)
from dtwin.benchmark.openswisshcc_candidate_volume_fallback import (
    _gallery_candidates,
    _validate_timing_plan,
)
from dtwin.benchmark.openswisshcc_candidate_volume_timing import SCENARIOS
from dtwin.core import PipelineError
from dtwin.medgemma_screening import _write_json_atomic


def _selected_timing_cases(plan: dict[str, Any]) -> list[dict[str, Any]]:
    selected = plan.get("selected_cases")
    if not isinstance(selected, list) or len(selected) != len(SCENARIOS):
        raise PipelineError("Plano temporal v16 nao possui os quatro casos criticos.")
    if [item.get("scenario") for item in selected] != list(SCENARIOS):
        raise PipelineError("Ordem/cenarios do piloto temporal v16 divergiram.")
    case_ids = [str(item.get("case_id", "")) for item in selected]
    if len(case_ids) != len(set(case_ids)) or any(not case_id.startswith("anon-") for case_id in case_ids):
        raise PipelineError("Casos do piloto temporal v16 invalidos ou duplicados.")
    expected_counts = {"fallback": 1, "one_candidate": 1, "three_candidates": 3, "five_candidates": 5}
    for item in selected:
        if (
            item.get("alignment_available") is not True
            or item.get("alignment_unavailable_reason") is not None
            or item.get("candidate_stack_count") != expected_counts[item["scenario"]]
            or item.get("fallback_no_candidate") is not (item["scenario"] == "fallback")
        ):
            raise PipelineError("Metadados do caso critico temporal v16 divergiram.")
    return selected


def build_candidate_volume_timing_gallery(
    *,
    timing_plan_path: Path,
    localizer_run: Path,
    input_manifest: Path,
    input_root: Path,
    registration_root: Path,
    output_root: Path,
    expected_source_case_count: int = 88,
    roi_mm: float = ROI_MM,
    output_side: int = OUTPUT_SIDE,
    max_input_bytes: int = 8_000_000,
) -> dict[str, Any]:
    plan_path = Path(timing_plan_path).resolve()
    plan = _validate_timing_plan(plan_path)
    selected = _selected_timing_cases(plan)
    localizer_run = Path(localizer_run).resolve()
    localizer_summary = _load(localizer_run / "summary.json")
    case_ids = [item["case_id"] for item in selected]
    if (
        not _valid_localizer_run_schema(localizer_summary)
        or localizer_summary.get("status") != "complete_scores_only_no_decision"
        or localizer_summary.get("ground_truth_read") is not False
        or localizer_summary.get("ground_truth_lesion_mask_used") is not False
        or localizer_summary.get("final_decision") is not None
        or any(case_id not in localizer_summary.get("case_ids", []) for case_id in case_ids)
    ):
        raise PipelineError("Run cego do localizador invalido para piloto temporal v16.")
    morphology = _input_index(input_manifest, input_root)
    dynamic = _original_dynamic_inputs(input_manifest, input_root)
    if len(morphology) != expected_source_case_count or set(morphology) != set(dynamic):
        raise PipelineError("Coorte fonte do piloto temporal v16 inesperada ou inconsistente.")

    destination = Path(output_root).resolve()
    if destination.exists():
        raise PipelineError("Destino do piloto temporal v16 ja existe; sobrescrita recusada.")
    destination.parent.mkdir(parents=True, exist_ok=True)
    staging = destination.parent / f"._v16timing_{uuid.uuid4().hex[:8]}"
    staging.mkdir()
    records = []
    try:
        for selection in selected:
            case_id = selection["case_id"]
            registered = _registered_or_none(case_id, registration_root)
            if registered is None:
                raise PipelineError("Caso critico temporal v16 perdeu o alinhamento publicado.")
            case_dir = staging / case_id
            case_manifest = build_candidate_volume_case(
                case_id=case_id,
                morphology_source=morphology[case_id],
                dynamic_source=dynamic[case_id],
                registered_source=registered,
                localizer_dir=localizer_run / case_id,
                destination=case_dir,
                roi_mm=roi_mm,
                output_side=output_side,
                max_input_bytes=max_input_bytes,
            )
            if (
                case_manifest.get("dynamic_alignment_mode") != "registered_to_venous"
                or case_manifest.get("candidate_stack_count") != selection["candidate_stack_count"]
                or case_manifest.get("selection", {}).get("fallback_no_candidate") is not selection["fallback_no_candidate"]
            ):
                raise PipelineError("Stack do caso critico divergiu do plano temporal v16 congelado.")
            gallery = _gallery_candidates(case_id, case_dir, case_manifest)
            for candidate in gallery:
                candidate["description"] = f'{selection["scenario"]}: {candidate["description"]}'
            records.append(
                {
                    "case_id": case_id,
                    "scenario": selection["scenario"],
                    "candidate_stack_count": case_manifest["candidate_stack_count"],
                    "case_manifest_sha256": _sha256(case_dir / "case_manifest.json"),
                    "dynamic_alignment_mode": case_manifest["dynamic_alignment_mode"],
                    "gallery_candidates": gallery,
                }
            )
        page = _gallery_page(records).replace(
            "As imagens enviadas ao modelo",
            "Esta galeria contem os quatro cenarios criticos congelados para o gate de 180 segundos. "
            "As imagens enviadas ao modelo",
            1,
        )
        (staging / "index.html").write_text(page, encoding="utf-8")
        cohort = {
            "schema": COHORT_SCHEMA,
            "contract": CONTRACT,
            "case_count": len(records),
            "candidate_stack_count": sum(item["candidate_stack_count"] for item in records),
            "cases": records,
            "source_timing_plan_sha256": _sha256(plan_path),
            "source_timing_plan_signature": plan["plan_signature"],
            "source_localizer_summary_sha256": _sha256(localizer_run / "summary.json"),
            "input_manifest_sha256": _sha256(Path(input_manifest).resolve()),
            "protocol": {
                "scenario_order": list(SCENARIOS),
                "roi_mm": roi_mm,
                "output_side": output_side,
                "dynamic_alignment_mode": "registered_to_venous",
                "candidate_target_fraction": TARGET_CANDIDATE_COVERAGE,
                "minimum_base_candidates": MIN_BASE_CANDIDATES,
                "maximum_candidates": MAX_CANDIDATES,
                "case_time_gate_seconds": 180.0,
            },
            "gallery_signature": _canonical(records),
            "technical_review_status": "pending",
            "timing_execution_started": False,
            "development_results_classification": "exploratory_only",
            "inference_executed": False,
            "ground_truth_read": False,
            "dataset_lesion_mask_used": False,
            "holdout_opened": False,
            "research_only": True,
            "clinical_use_allowed": False,
            "requires_human_review": True,
        }
        _write_json_atomic(staging / "cohort_manifest.json", cohort)
        _publish_directory(staging, destination)
        return cohort
    except Exception:
        shutil.rmtree(staging, ignore_errors=True)
        raise
