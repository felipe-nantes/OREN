"""Render the label-blind CHAOS v21 uniform-9 technical gallery."""
...
CASE_SCHEMA = "argos-chaos-v21-uniform9-panel-case-v1"
COHORT_SCHEMA = "argos-chaos-v21-uniform9-panel-cohort-v1"
GALLERY_SCHEMA = "argos-chaos-v21-uniform9-gallery-v1"
ROLE_TO_PHASE = {"t1in": "t1_in", "t1out": "t1_out", "t2spir": "t2_spir"}
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise PipelineError(f"JSON CHAOS v21 invalido: {path}") from exc
    if not isinstance(value, dict):
        raise PipelineError(f"JSON CHAOS v21 nao e objeto: {path}")
    return value
...
def _safe_relative(root: Path, relative: str) -> Path:
    ...
    if posix.is_absolute() or ".." in posix.parts:
        raise PipelineError("Caminho preparado CHAOS v21 inseguro.")
    path = (root / Path(*posix.parts)).resolve()
    if not path.is_relative_to(root):
        raise PipelineError("Caminho preparado CHAOS v21 escapou da raiz.")
    return path
...
def _validate_config(path: Path) -> dict[str, Any]:
    ...
    channel_map = panel.get("fusion", {}).get("channel_map", {})
    prompt = str(config.get("prompt", {}).get("template", "")).lower()
    if (
        panel.get("mode") != "multiphase_fusion"
        or panel.get("strategy") != "uniform_9"
        or int(panel.get("axial_slices", 9)) != 9
        or channel_map != {"red": "t1in", "green": "t1out", "blue": "t2spir"}
        ...
        or "nao sao fases dinamicas" not in prompt
    ):
        raise PipelineError("Config CHAOS v21 nao preserva o leitor choice 4B e a semantica T1/T2.")
    return config
...
def _case_files(prepared_root: Path, case_record: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Path]]:
    ...
    if not manifest_path.is_file() or _sha256(manifest_path) != case_record.get("case_manifest_sha256"):
        raise PipelineError("Manifesto preparado CHAOS v21 ausente ou adulterado.")
    ...
    if (
        ...
        raise PipelineError("Manifesto preparado CHAOS perdeu isolamento de labels/lesao.")
    files: dict[str, Path] = {}
    for item in manifest.get("files", []):
        ...
        if role in files or not path.is_file() or _sha256(path) != item.get("sha256"):
            raise PipelineError("Arquivo preparado CHAOS ausente, duplicado ou adulterado.")
        if "tumor" in path.name.lower() or "lesion" in path.name.lower():
            raise PipelineError("Arquivo de lesao apareceu no workspace CHAOS.")
        files[role] = path
    if set(ROLE_TO_PHASE.values()) | {"liver_mask"} != set(files):
        raise PipelineError("Caso CHAOS nao possui exatamente T1 in/out, T2-SPIR e mascara hepatica.")
    return manifest, files


def build_chaos_uniform9_panels(
    *, prepared_root: Path, output_root: Path, config_path: Path, profile_path: Path,
    expected_case_count: int = 20, expected_prepared_signature: str | None = None,
) -> dict[str, Any]:
    """Render all CHAOS cases atomically; outputs require human review."""
    ...
    if output_root.exists():
        raise PipelineError("Coorte de paineis CHAOS v21 ja existe.")
    output_root.parent.mkdir(parents=True, exist_ok=True)
    staging = output_root.parent / f"._chaos_v21_panels_{uuid.uuid4().hex[:8]}"
    staging.mkdir()
    ...
    try:
        for case_record in cohort["cases"]:
            ...
            result = generate_liver_panel_multiphase(
                ...
                organ_profile=load_profile(profile_path), screening_config=config,
                output_dir=case_dir, model_trace=model_trace(config),
            ...
            if (
                ...
                or panel_manifest.get("fusion_channel_map") != {
                    "red": "t1in", "green": "t1out", "blue": "t2spir"
                }
            ):
                raise PipelineError("Painel CHAOS v21 violou representacao, cegamento ou gate visual.")
            with Image.open(result.panel_path) as image:
                width, height = image.size
                metadata_keys = sorted(image.info)
            if metadata_keys:
                raise PipelineError("PNG CHAOS v21 contem metadados inesperados.")
            ...
            manifest = {
                ...
                "source_case_signature": source_manifest["case_signature"],
                "panel_filename": result.panel_path.name, "panel_sha256": panel_sha,
                ...
                "panel_manifest_sha256": _sha256(result.manifest_path),
                "sequence_semantics": {"red": "T1 in-phase", "green": "T1 out-phase", "blue": "T2-SPIR"},
                "visible_phi_confirmed": False, "eligible_for_inference": False,
                "lesion_mask_used": False, "pathology_label_used": False,
            ...
            records.append({
                ...
                "candidate_signature": signature,
                "panel": f"{case_id}/{result.panel_path.name}", "panel_sha256": panel_sha,
            })
        summary = {
            "schema": COHORT_SCHEMA, "status": "complete_pending_human_review",
            "evaluation_scope": "secondary_negative_domain_shift_stress_only",
            "case_count": len(records), "case_ids": [record["case_id"] for record in records],
            "cases": records, "prepared_cohort_signature": preflight["cohort_signature"],
            "prepared_cohort_manifest_sha256": _sha256(cohort_path),
            "config_sha256": _sha256(config_path), "profile_sha256": _sha256(profile_path),
            "all_panels_uniform9": True, "all_panels_pending_human_review": True,
            "sequence_semantics": {"red": "T1 in-phase", "green": "T1 out-phase", "blue": "T2-SPIR"},
...
def build_chaos_uniform9_gallery(*, panel_root: Path, output_dir: Path) -> dict[str, Any]:
    """Create a local label-free technical gate for CHAOS panels."""
    ...
    if (
        ...
        raise PipelineError("Coorte CHAOS v21 nao esta pronta para galeria tecnica.")
    if output_dir.exists():
        raise PipelineError("Galeria CHAOS v21 ja existe.")
    output_dir.parent.mkdir(parents=True, exist_ok=True)
    staging = output_dir.parent / f"._chaos_v21_gallery_{uuid.uuid4().hex[:8]}"
    staging.mkdir()
    try:
        cards: list[str] = []
        ...
        for number, record in enumerate(cohort["cases"], start=1):
            ...
            if not source.is_file() or _sha256(source) != record["panel_sha256"]:
                raise PipelineError("Painel CHAOS v21 ausente ou adulterado antes da galeria.")
            filename = f"{number:02d}_{case_id}.png"
            shutil.copy2(source, staging / filename)
            if _sha256(staging / filename) != record["panel_sha256"]:
                raise PipelineError("Copia da galeria CHAOS divergiu do painel fonte.")
            cards.append(
                '<article class="card">'
                f'<h2>{number}. {html.escape(case_id)}</h2>'
                f'<img src="{html.escape(filename)}" alt="Painel tecnico {number}">'
                '<p>Avalie somente qualidade tecnica: figado visivel e completo, crop e orientacao plausiveis, '
                'fusao interpretavel (R=T1 in, G=T1 out, B=T2-SPIR), contorno sem ocultar parenquima e ausencia de PHI.</p>'
                '</article>'
            )
            gallery_cases.append({
                "number": number, "case_id": case_id, "image": filename,
                "sha256": record["panel_sha256"],
            })
        document = """<!doctype html><html lang="pt-BR"><head><meta charset="utf-8">
<title>ARGOS v21 — CHAOS, gate técnico cego</title><style>
...
<h1>ARGOS v21 — galeria técnica cega CHAOS</h1>
<div class="notice"><b>Não avalie diagnóstico.</b> Confirme somente a qualidade técnica e a correspondência
R=T1 in-phase, G=T1 out-phase, B=T2-SPIR. Não são fases dinâmicas. Nenhum rótulo patológico ou máscara de lesão
está disponível. Os painéis permanecem inelegíveis para inferência até aprovação explícita.</div>
