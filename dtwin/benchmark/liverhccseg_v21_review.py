"""Signed human technical review gate for LiverHccSeg v21 panels."""
...
REVIEW_SCHEMA = "argos-liverhccseg-v21-uniform9-human-review-v1"


def _load(path: Path) -> dict[str, Any]:
    if not Path(path).is_file():
        raise PipelineError(f"Revisao humana v21 ausente: {path}")
    try:
        value = json.loads(Path(path).read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise PipelineError(f"JSON de revisao v21 invalido: {path}") from exc
    if not isinstance(value, dict):
        raise PipelineError("Artefato de revisao v21 deve ser objeto.")
    return value
...
def _safe(root: Path, relative: str) -> Path:
    ...
    if part.is_absolute() or ".." in part.parts:
        raise PipelineError("Caminho de painel v21 inseguro.")
    path = (root / Path(*part.parts)).resolve()
    if not path.is_relative_to(root):
        raise PipelineError("Caminho de painel v21 saiu da raiz.")
    return path
...
def _validate_sources(panel_root: Path, gallery_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise PipelineError("Coorte de paineis v21 invalida ou adulterada.")
    if (
        ...
        raise PipelineError("Galeria v21 invalida ou adulterada.")
    ...
    if not isinstance(cohort_cases, list) or not isinstance(gallery_cases, list):
        raise PipelineError("Casos da coorte/galeria v21 ausentes.")
    if [row.get("case_id") for row in cohort_cases] != [row.get("case_id") for row in gallery_cases]:
        raise PipelineError("Ordem de casos da galeria v21 diverge da coorte.")
    for cohort_case, gallery_case in zip(cohort_cases, gallery_cases, strict=True):
        ...
        if (
            not panel.is_file()
            or not gallery_image.is_file()
            or _sha256(panel) != expected
            or _sha256(gallery_image) != expected
            or gallery_case.get("sha256") != expected
        ):
            raise PipelineError("Hash de painel/galeria v21 divergiu antes da aprovacao.")
    return cohort, gallery


def create_liverhccseg_v21_review(
    *,
    panel_root: Path,
    gallery_root: Path,
    output_path: Path,
    ...
    """Persist an explicit all-case approval; partial approval is not accepted."""

    reviewer = str(reviewer).strip()
    if approved is not True:
        raise PipelineError("A inferencia v21 exige aprovacao humana explicita.")
    if not reviewer or len(reviewer) > 80:
        raise PipelineError("Identificacao do revisor v21 ausente ou invalida.")
    if len(str(note)) > 1000:
        raise PipelineError("Nota de revisao v21 excede 1000 caracteres.")
    panel_root = Path(panel_root).resolve()
    gallery_root = Path(gallery_root).resolve()
    ...
    payload: dict[str, Any] = {
        ...
        "case_count": int(cohort["case_count"]),
        "note": str(note).strip(),
        "checks_confirmed": {
            "liver_visible": True,
    ...
    if output.exists():
        raise PipelineError("Revisao humana v21 ja existe; recuso sobrescrever.")
    output.parent.mkdir(parents=True, exist_ok=True)
    _write_json_atomic(output, payload)
    return payload


def verify_liverhccseg_v21_review(
    *, panel_root: Path, gallery_root: Path, review_path: Path, expected_reviewer: str | None = None
) -> dict[str, Any]:
    ...
    if (
        ...
        raise PipelineError("Revisao humana v21 invalida, incompleta ou adulterada.")
    if expected_reviewer is not None and review.get("reviewer") != expected_reviewer:
        raise PipelineError("Revisor v21 diverge do esperado.")
    return review
