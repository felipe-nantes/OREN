...
|---|---|---|---|
| SW-ATOMIC-01 | Auditoria 2026-08-18: **56 helpers mapeados**; 3 escritores canônicos sondados empiricamente — destino nunca fica parcial na interrupção. Divergência de higiene registrada em TD-007 (`reporting._atomic_text` vaza o temporário) | **tests/test_property_atomic_and_artifact.py** (property + auditoria estrutural AST; PHASE_04 wave 3) | VERIFIED_BY_PROPERTY (gap fechado em 2026-08-18) |
...
| SW-ARTIFACT-01 | `learning/visual_inference.py:78-84` (assinatura+hash do bundle); `webapp/server.py:880-884` (`sha256_of` por artefato); `volumetry.py:558-583` (verificador independente) | tests/test_learning_visual_inference.py:30-73 + **tests/test_property_atomic_and_artifact.py** (property: adulteração em qualquer byte muda o hash; incompleto/corrompido recusado) | VERIFIED_BY_PROPERTY (PHASE_04 wave 3) |
...
| GEO-CONVERT-01 | `core.py:90` (`CopyInformation` na conversão) | tests/test_core_geometry.py + **tests/test_property_geometry_roundtrip.py** (property test Hypothesis, 400 exemplos; PHASE_04 wave 1) | VERIFIED_BY_PROPERTY (gap fechado em 2026-08-18) |
...
| GEO-LABEL-01 | Auditoria exaustiva 2026-08-18: **28 call sites em `dtwin/`, zero violações** (discreto→nearest, contínuo→linear) | tests/test_learning_multiphase_ingest.py + **tests/test_property_label_resampling.py** (property + auditoria estrutural AST; PHASE_04 wave 2) | VERIFIED_BY_PROPERTY (gap fechado em 2026-08-18) |
